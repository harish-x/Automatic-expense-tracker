...
export async function syncTransactions(): Promise<SyncResult> {
  try {
    ...
    if (!token) return { synced: 0, error: 'Not authenticated', authExpired: true };

    const syncedIds: number[] = [];

    for (const tx of transactions) {
      try {
        await postExpenseMetric(tx.rawSms, token);
        if (tx.id != null) syncedIds.push(tx.id);
      } catch (err: unknown) {
        // On 401 try refreshing the token once, then retry this message
        if (axios.isAxiosError(err) && err.response?.status === 401) {
          const newToken = await refreshAccessToken();
          if (!newToken) return { synced: syncedIds.length, error: 'Session expired. Please log in again.', authExpired: true };
          token = newToken;
...
function postExpenseMetric(message: string, token: string) {
  return axios.post(
